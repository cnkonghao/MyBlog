(function(e) {
	if (!Function.prototype.bind) Function.prototype.bind = function(c) {
		...
		f = function() {},
		g = function() {
			return d.apply(this instanceof f ? this: c || {},
	...
	e.Code.Util = {
		registerNamespace: function() {
			...
			for (g = c.length; b < g; b++) {
				...
				a = e[a];
				d = 1;
				for (h = f.length; d < h; ++d) a[f[d]] = a[f[d]] || {},
				a = a[f[d]]
			}
		},
		coalesce: function() {
		...
		Events: {
			...
			fire: function(a, b) {
				...
				if (a.__eventHandlers[d.type] instanceof Array) {
					...
					j.unshift(d);
					d = 0;
					for (f = g.length; d < f; d++) h = g[d],
					c.isNothing(h) || h.apply(a, j)
				}
			},
	...
	c.extend(c, {
		DOM: {
			...
			_setStyle: function(a, b, d) {
				var f;
				if (c.isObject(b)) for (f in b) c.objectHasProperty(b, f) && (f === "width" ? c.DOM.width(a, b[f]) : f === "height" ? c.DOM.height(a, b[f]) : a.style[f] = b[f]);
				else a.style[b] = d
			},
			...
			width: function(a, b) {
				if (!c.isNothing(b)) c.isNumber(b) && (b += "px"),
				a.style.width = b;
	...
	c.extend(c, {
		Animation: {
			...
			_leaveTransforms: function(a) {
				...
				f,
				b = c.coalesce(a.style.webkitTransform, a.style.MozTransform, a.style.transform),
				g,
...
(function(e, c, a) {
	...
	a.TouchElement.TouchElementClass = c({
		...
		onTouchStart: function(b) {
			...
			d.length > 1 && this.captureSettings.gesture ? this.isGesture = !0: (this.touchStartTime = new Date, this.isGesture = !1, this.touchStartPoint = this.getTouchPoint(d), a.Events.fire(this, {
				type: a.TouchElement.EventTypes.onTouch,
				target: this,
		...
		onMouseOut: function(b) {
			...
			if (! (this.el === d || a.DOM.isChildOf(d, this.el))) b.preventDefault(),
			this.captureSettings.move && a.Events.remove(this.el, "mousemove", this.mouseMoveHandler),
			a.Events.remove(this.el, "mouseup", this.mouseUpHandler),
			a.Events.remove(this.el, "mouseout", this.mouseOutHandler),
			this.touchEndPoint = a.Events.getMousePosition(b),
	...
	b.Cache.CacheClass = c({
		...
		getImages: function(d) {
			...
			if (this.settings.cacheMode === b.Cache.Mode.aggressive) {
				f = 0;
				for (c = this.images.length; f < c; f++) j = this.images[f],
				a.objectHasProperty(j, "cacheDoNotShrink") ? delete j.cacheDoNotShrink: j.shrinkImage()
			}
			return e
		}
	})
...
(function(e, c, a) {
	...
	b.DocumentOverlay.DocumentOverlayClass = c({
		...
		resetPosition: function() {
			...
			} else b = a.DOM.width(this.settings.target),
...
(function(e, c, a) {
	...
	b.Carousel.CarouselClass = c({
		...
		dispose: function() {
			...
			c;
			f = 0;
			for (c = this.cache.images.length; f < c; f++) a.Events.remove(this.cache.images[f], b.Image.EventTypes.onLoad, this.imageLoadHandler),
			a.Events.remove(this.cache.images[f], b.Image.EventTypes.onError, this.imageErrorHandler);
			this.stopSlideshow();
			a.Animation.stop(this.el);
			a.DOM.removeChild(this.el, this.el.parentNode);
			for (d in this) a.objectHasProperty(this, d) && (this[d] = null)
		...
		slideCarousel: function(d, c, g) {
			if (!this.isSliding) {
				...
				if (! (e.Math.abs(j) < 1)) {
					...
					if (!this.settings.loop && (this.lastSlideByAction === b.Carousel.SlideByAction.previous && this.currentCacheIndex === 0 || this.lastSlideByAction === b.Carousel.SlideByAction.next && this.currentCacheIndex === this.cache.images.length - 1)) d = 0,
		...
		onSlideByEnd: function() {
			if (!a.isNothing(this.isSliding)) {
				...
				if (this.settings.loop) if (this.lastSlideByAction === b.Carousel.SlideByAction.next ? a.DOM.appendChild(d[0], this.contentEl) : this.lastSlideByAction === b.Carousel.SlideByAction.previous && a.DOM.insertBefore(d[d.length - 1], d[0], this.contentEl), this.currentCacheIndex < 0) this.currentCacheIndex = this.cache.images.length - 1;
				else {
...
(function(e, c, a) {
	...
	c.Carousel.CarouselClass = c.Carousel.CarouselClass.extend({
		...
		doMoveCarousel: function(b) {
			var d;
			a.Browser.isCSSTransformSupported ? (d = {},
			d[a.Animation._transitionPrefix + "Property"] = "all", d[a.Animation._transitionPrefix + "Duration"] = "", d[a.Animation._transitionPrefix + "TimingFunction"] = "", d[a.Animation._transitionPrefix + "Delay"] = "0", d[a.Animation._transformLabel] = a.Browser.is3dSupported ? "translate3d(" + b + "px, 0px, 0px)": "translate(" + b + "px, 0px)", a.DOM.setStyle(this.contentEl, d)) : a.isNothing(e.jQuery) || e.jQuery(this.contentEl).stop().css("left", this.getStartingPos().x + b + "px")
		},
		doSlideCarousel: function(b, d) {
			var c;
			if (d <= 0) this.slideByEndHandler();
			else if (a.Browser.isCSSTransformSupported) c = a.coalesce(this.contentEl.style.webkitTransform, this.contentEl.style.MozTransform, this.contentEl.style.transform, ""),
...
(function(e, c, a) {
	...
	b.Toolbar.ToolbarClass = c({
		...
		handleTap: function(d) {
			...
			else if (d.target === this.playEl || a.DOM.isChildOf(d.target, this.playEl)) c = b.Toolbar.ToolbarAction.play;
			this.setTimeout();
...
(function(e, c) {
	...
	a.attach = function(b, d, e) {
		...
		d = 0;
		for (e = b.length; d < e; d++) if (h = b[d], !c.isNothing(h.nodeType) && h.nodeType === 1) h.__photoSwipeClickHandler = a.onTriggerElementClick.bind(g),
	...
	a.detatch = function(b) {
		...
		g;
		d = 0;
		for (e = b.originalImages.length; d < e; d++) g = b.originalImages[d],
		!c.isNothing(g.nodeType) && g.nodeType === 1 && (c.Events.remove(g, "click", g.__photoSwipeClickHandler), delete g.__photoSwipeClickHandler);
		a.disposeInstance(b)
	};
	a.createInstance = function(b, d, e) {
		...
		d = c.coalesce(d, {});
		g = a.getInstance(e);
		if (c.isNothing(g)) g = new a.PhotoSwipeClass(b, d, e),
		a.instances.push(g);
	...
	a.getInstanceIndex = function(b) {
		...
		g = -1;
		c = 0;
		for (e = a.instances.length; c < e; c++) if (a.instances[c] === b) {
			g = c;
			break
		}
		return g
...
(function(e, c, a, b, d, f, g, h, j) {
	...
	i.PhotoSwipeClass = c({
		...
		addEventHandlers: function() {
			...
			a.Events.add(e.document, "click", this.clickHandler);
		...
		removeEventHandlers: function() {
			...
			a.Events.remove(e.document, "click", this.clickHandler);
		...
		createZoomPanRotate: function() {
			this.stop();
			if (this.canUserZoom() && !this.isZoomActive()) a.Events.fire(this, i.EventTypes.onBeforeZoomPanRotateShow),
			...
			a.Events.add(this.zoomPanRotate, i.ZoomPanRotate.EventTypes.onTransform, this.zoomPanRotateTransformHandler),
		...
		destroyZoomPanRotate: function() {
			if (!a.isNothing(this.zoomPanRotate)) a.Events.fire(this, i.EventTypes.onBeforeZoomPanRotateHide),
			a.Events.remove(this.zoomPanRotate, i.ZoomPanRotate.EventTypes.onTransform, this.zoomPanRotateTransformHandler),
